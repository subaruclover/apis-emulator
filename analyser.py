...
class analyserClass:
    def __init__(self):
        ...
        self.indivLog = []
        self.summaryLog = []
        # TODO: why set lastAvgRSOC to 50?
        self.lastAvgRSOC = 50
        self.sumUntilNow = []
    ...
    def analyseAndLog(self):
        ...
        if conf.saveIndividualToDB:
            ...
            for i in extendedOesCopy:
                extendedOesCopy[i]["emul"] = {"acloss": gl.acloss[i], "dcloss": gl.dcloss[i],
                                              "loss": gl.dcloss[i] + gl.acloss[i], "wasted": gl.wasted[i]}
    ...
    def summaryToMemory(self):
        ...
        self.ratio["deltaBatt"] = int(
            float(self.cumul["avgrsoc"] - self.lastAvgRSOC) * sum(conf.batterySize.values()) / 100)
        # self sufficiency in reality
        self.ratio["ssr_real"] = round(
        ...
        if len(self.sumUntilNow) == 0:  # initalize
            ...
            with open("data/output/daily_output_prior_iter3_run3_time.txt", "w") as f:
        ...
        with open("data/output/daily_output_prior_iter3_run3_time.txt", "a") as f:
            print(l, file=f)
    ...
    def writeToCSV(self):
        ...
        if conf.saveToSummaryToCSV:
            ...
            with open(conf.summaryPath, 'w') as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["time", "pv", "demand", "acin", "wasted", "acloss", "dcloss", "loss", "avgrsoc", "wg", "deltaBatt",
                     "ssr_real", "ssr_pv", "sor", "r_utility"])
